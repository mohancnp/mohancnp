...
class ProductServiceImpl extends ProductService {
  ...
  Future handleProductsOfType({required String type}) async {
    try {
      Map<String, dynamic> products = await remoteSource
          .get('${AppConfig.baseUrl}/api/product', queryParams: {"type": type});
      return products;
  ...
  Future handleAllProducts() async {
    try {
      Map<String, dynamic> products =
          await remoteSource.get('${AppConfig.baseUrl}/api/product');
      return products;
    } on ServerException catch (e) {
      throw AppException(message: e.message);
    }
  ...
  Future handleProductDetail({required int id}) async {
    try {
      Map<String, dynamic> productDetail =
          await remoteSource.get('${AppConfig.baseUrl}/api/product/$id');
  ...
  Future getFavoriteProducts() async {
    try {
      var products =
          await remoteSource.get('${AppConfig.baseUrl}/api/favourite');
      return products;
    } on ServerException catch (e) {
      throw (AppException(message: e.message));
    }
  ...
  Future toggleFavoriteProduct({required int id}) async {
    try {
      var products =
          await remoteSource.post('${AppConfig.baseUrl}/api/favourite/$id');
      return products;
    } on ServerException catch (e) {
      throw (AppException(message: e.message));
    }
  ...
  Future getPublicProducts() async {
    try {
      ...
      var products = await dio.get('${AppConfig.baseUrl}/api/product');
  ...
  Future getPublicProductDetail({required int id}) async {
    try {
      var token =
          "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..mHPXsWSDaFiqNFYZLAtEhiFxdldBDE0W4IXYCg-n4HcG9sr9yZud9PoOKvXnV4qFl-eWECReojYeIOTle_r-5C0D9q93c5sqvv7b3lyzWQUNvP4NuP8EqpjbZm9b3nChZpR_VFSqwPBFYjz8S8cHwAQaGlgI78-k3O3SN7iUUVsn2rZBlE109GfG4B0hai5NsSj0RJtHoR79Pic7DXLWt1aLCYybVo3Bm7l70dLLJF5qlmDZwKB6H8rOHMsyJaZzsDMovECaYkowfndeW8p5vguBXgwwIG81rxTPUXVThkTcsYmglkaRZI1HF9MWSvblyeOiQr26-KMOHznejAauNzfY0c0k3cpMqoS89fQ024mVxeZHYm9loN_YSrYVjXKcFzqEBaHz8-_5XTcwZJKU-LVLjUMa_jq0cLvey4GDZoH6Z36exDGD3eqsYvXE6Buih4vfzPalyP-UxAZp_7woGVlgH-d0SViT9Zpw0OZuaCbhkQvABCK-izpm32ndJnRCUwmA5tBELwFE7saOrUHhY08C6XXnmtDDRUMy2-tezPjj4B32vvzJPn9iJgW1oXuFRD19xL9yvoA2aA4rHAcb4QNke7wXadeASFhA0U6-mbyO2vqxJoBTinzZgJonBllhbottq7sev6jFQoM3IzQU9kwQ_NHkn7g8COP7DjKzq9E";
      dio.options.headers[HttpHeaders.authorizationHeader] = 'Bearer $token';
      var productDetail = await dio.get('${AppConfig.baseUrl}/api/product/$id');
  ...
  Future<Either<List<Category>, Failure>> getCatoriesList() async {
    ...
    try {
      ...
      var response = await remoteSource.get("${AppConfig.baseUrl}/api/category_list");
  ...
  Future<Either<CategoryProduct, Failure>> getProductForCategory({
    ...
    try {
      var response = await remoteSource.get("${AppConfig.baseUrl}/api/cat_prod_list/$id");
  ...
  Future<Either<ProductDetail, Failure>> getProductDetailWithId(int id) async {
    ...
    try {
      var data = await remoteSource.get(
        "${AppConfig.baseUrl}/api/product_detail/$id",
      );
  ...
  Future<Either<FilterProduct, Failure>> getFilteredList(String filter) async {
    ...
    try {
      var data = await remoteSource.get("${AppConfig.baseUrl}/api/product_list_filter",
